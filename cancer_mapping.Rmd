...
wa$region <- 1:nrow(wa)
# Running INLA with given specifications (no covariates)
...
```{r BYM2-plot1, echo=TRUE, fig.height=6.5, fig.cap="Posterior Medians for BYM2 Model", eval=T}
pal = function(n) brewer.pal(n,"BuPu")
...
p1 <- ggplot(data.frame(psd=wa$fit1se,se=wa$se),
       aes(y=psd,x=se)) + geom_point() + labs(y="BYM2 Standard Deviation",x="SMR Standard Error", 
                                              title='BYM2 Posterior Standard Deviations vs. Standard Errors of SMRs') +
...
- Compared to the SMR Standard Error and the Posterior St Devs. from the IID model, the BYM2 model has the lowest errors. We can observe shrinkage, which is most apparent for high error areas

- In General BYM2 is pretty aligned to both the SMR/IID errors, but does well too reduce the errors for high error areas, presumably due to spatial smoothing using neighbor matrix.

```{r, fig.height=4, fig.width=4}
# Plotting Posterior Median vs SMR
ggplot(data.frame(pmedian=model.fit1$summary.fitted.values$`0.5quant`,SMR=wa$SMR),
       aes(y=pmedian,x=SMR, size = wa$se)) + geom_point(alpha = 0.5) + labs(y="Posterior Median",x="SMR") + 
...
formula <- Count ~ 1 + PM.Concentration + f(region, model="bym2", graph=mat, scale.model=T, constr=T,
                         hyper=list(phi=list(prior="pc", param=c(0.5, 0.5), initial=1), prec=list(prior="pc.prec", param=c(0.3,0.01),initial=5)))
...
The posterior median of $\sigma_e$ is ``r 1/sqrt(model.fit2$summary.hyperpar[1, 4])`` with a 95% interval of [``r c(1/sqrt(model.fit2$summary.hyperpar[1, 5])), 1/sqrt(model.fit2$summary.hyperpar[1, 3]))``].